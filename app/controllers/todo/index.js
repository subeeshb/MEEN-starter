...
module.exports = {
    ...
    addItem: function(itemText, callback) {
        ...
        var todoItem = new models.TodoItem(itemID, itemText);
        db.todo.save(todoItem, function(err, saved) {
            if(err || !saved) {
    ...
    getTodoItems: function(callback) {
        ...
        db.todo.find(function(err, matches) {
            if(err || !matches) {
    ...
    deleteItem: function(itemID, callback) {
        ...
        db.todo.remove({itemID: itemID}, function(err, deleted) {
            if(err || !deleted) {
